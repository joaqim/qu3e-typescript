...
export abstract class Collide {
  ...
  public static results = Array.from({ length: 8 });
  public static depths = Array.from({ length: 8 });
  ...
  public static BoxtoBox(m: Manifold, a: Box, b: Box): void {
    ...
    trackFaceResult = this.TrackFaceAxis(
      aAxis,
      1,
      s,
      aMax,
    ...
    if (!parallel) {
      ...
      // Cross( a.x, b.x )
      let rA = extentA.y * absC.GetByAxis(0, 2) + extentA.z * absC.GetByAxis(0, 1);
      let rB = extentB.y * absC.GetByAxis(2, 0) + extentB.z * absC.GetByAxis(1, 0);
    ...
    if (Vec3.Dot(n, btx.position.Sub(atx.position)) < 0) n = n.Inverse();
    ...
    if (axis < 6) {
      ...
      if (resultNumber != 0) {
        ...
        for (var index = 0; index < resultNumber; ++index) {
      ...
      const { a: PB, b: QB } = this.SupportEdge(btx, extentB, n.Inverse());
