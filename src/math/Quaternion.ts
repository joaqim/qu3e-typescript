...
export default class Quaternion {
    ...
    constructor(payload: [x: number, y: number, z: number, w: number] | [axis: Vec3, radians: number]) {
        if (payload.length == 4) {
            this.SetRow(...payload)
        } else {
            this.x = 0
    ...
    ToAxisAngle(axis: Vec3, angle: number): { axis: Vec3, angle: number } {
        ...
        var l = Math.sqrt(1 - this.w * this.w)
    ...
    Integrate(dv: Vec3, dt: number): void {
        var q = new Quaternion([dv.x * dt, dv.y * dt, dv.z * dt, 0])
     ...
     * Multiply
     */
    public static Multiply(lhs: Readonly<Quaternion>, rhs: Readonly<Quaternion>): Quaternion {
