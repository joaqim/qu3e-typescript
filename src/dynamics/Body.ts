...
import { BoxDefinition } from "@collision/BoxDefinition";
import type Box from "@collision/Box";
import { CreateBox } from "@collision/CreateBox";
import AABB from "@common/geometry/AABB";
import type Render from "@common/Render";
import type { ContactConstraint, ContactEdge } from "@dynamics";
...
export default class Body {
  ...
  public AddBox(def: BoxDefinition): Box {
    const box = CreateBox(def, this);
    this.Boxes.Add(box);
  ...
  ApplyTorque = (torque: ReadonlyVec3): void => {
  ...
  GetVelocityAtWorldPoint(p: Vec3): Vec3 {
    ...
    const relativeAngularVel = Vec3.Cross(
